...
import User from "@/assets/icon/user-profile-icon.jpg";
...
const Review: React.FC<ReviewProps> = ({
  ...
  return (
    <div className="bg-white hover:bg-gray-50 border-1 pb-2 border-gray-200 gap-4 p-3 rounded-lg flex justify-between">
      <div className="flex flex-1 items-center gap-4">
        <div className="flex flex-col gap-2">
          <img
            src={review?.userProfileImage !== null ? review?.userProfileImage : User}
            className="w-20 max-h-fit flex aspect-square rounded-full bg-gray-50 object-cover"
          ></img>
          <span className="text-sm text-center">{review?.username}</span>
        </div>

        <div className="flex flex-col gap-2">
          <p className="text-black text-base font-semibold"></p>
          <RatingStar initialRating={review?.ratingPoint ?? 0} readonly />
          ...
          <div className="flex flex-row items-center gap-4">
            ...
            <div className="flex flex-row items-center gap-4">
              <img
                ...
                className="w-20 max-h-fit flex aspect-square rounded-lg bg-gray-50 object-cover"
              ></img>
